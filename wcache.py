...
cache = []
cache = range(1024)
doc = open('/Users/sayalishirode/Documents/ECE658/Lab04/'+ str(ip), 'w+')
...
def connect_to_bootstrap():
    ...
    f = open("/Users/sayalishirode/Documents/ECE658/Lab04/resources.txt", "r")
    a = []
    for line in f:
        a.append(line.strip())
    index = [0,1,2]
...
class Receive(Thread):
    def run(self):
        global sock, neighbours, random_files, neighbour, ip, port, value, status,resources_at_nodes,doc
        hops = 0
	cache = []
	    ...
	    print "Received", msg, server
            ...
            #             del neighbours[i]
                ...
                #print msg
		resources_available = msg[2]+ " " + str(msg[3])+ " " + str(msg[5:])
		#resources_at_node.append(resources_available)
		print "resources : ", resources_available
		cache.append(resources_available)
		print "Available files: ",cache
		#print type(resources_at_node)
        	#print type(resources_available)
		...
		for line in cache:
			if query.lower().strip() in line.lower().strip():    
				total_search_nodes_reply = reply_search_nodes(msg[2],msg[3],query, hops)
				print "Query",query, "is available at -----",msg[2:4] 
				sock.sendto(total_search_nodes_reply,address)
				found = True
				break	
                if found is True:
...
class Queries(Thread):
    def run(self):
        ...
        g = open("/Users/sayalishirode/Documents/ECE 658/Lab04/nodes.txt", "r")
        b = []
        for line in g:
            #print line
            b.append(line.strip())
...
def run():
    ...
    resources = []
    #for x in range(number_of_queries):
    node_files = open("/Users/sayalishirode/Documents/ECE658/Lab04/nodes.txt", "r")
    resources_files = open("/Users/sayalishirode/Documents/ECE658/Lab04/resources.txt", "r")
    nodes = []
    ...
    for resource in resources:
        ...
        for i, node in enumerate(random_nodes):
            ...
            sock.sendto(message, (ip, random_senders[i]))
...
if __name__ == "__main__":
    parse_arguments()
    doc = open('/Users/sayalishirode/Documents/ECE658/Lab04/'+str(ip)+ ".log", 'w+')
    sock.bind(('', port))
